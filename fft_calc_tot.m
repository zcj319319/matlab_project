...
% 20211207
% add figure_overwrite for continues update data fft result

% 20211223
% add refclk_ratio to plot refclk related spur
% change SFDR vth default 70, also plot red font less not greater than vth
% REF SPUR also use SFDR vth, but use its negative

% 20220207
% add SIG1 angle
% add para.angle
% if ==1, perf.angle=SIG1 angle
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% dbc_th_HD dbc_th_IMD dbc_th_IL dbc_th_SFDR  threshold for whether highlight relative values
% ENOB_include_HD   whether ENOB include HD
% plot_option       plot figure or not
% figure_overwrite  if overwrite=1 figure will always use figure 1 to overwrite last plot
%% for adc
% % para.sigbw=100e6;
% % para.dpdbw=300e6;
% para.sideband=300;
...
res_index=setdiff([20:(N_nq/2+1)-20],index_collect);
[dummy,bx]=max(mag_all(res_index));  % bx is max index in res_index it self
bx=res_index(bx);                    % so get orignal index in res_index
index_center_spur=bx;
...
if imd_mode~=1
    if num_interleave~=1
        SNDR_dbc=10*log10(pow_SIG1/(pow_noise+sum(pow_HD_SIG1)+sum(pow_IL_OS)+sum(pow_IL_GTS_SIG1)+pow_spur));
    else
        SNDR_dbc=10*log10(pow_SIG1/(pow_noise+sum(pow_HD_SIG1)+pow_spur));
    end
else
    if num_interleave~=1
        SNDR_dbc=10*log10((pow_SIG1+pow_SIG2)/(pow_noise+sum(pow_HD_SIG1)+sum(pow_HD_SIG2)+sum(pow_IL_OS)+sum(pow_IL_GTS_SIG1)+sum(pow_IL_GTS_SIG2)+pow_spur));
    else
        SNDR_dbc=10*log10((pow_SIG1+pow_SIG2)/(pow_noise+sum(pow_HD_SIG1)+sum(pow_HD_SIG2)+pow_spur));
    end
end
if imd_mode~=1
...
if plot_option==1
        ...
        for n=1:length(index_center_IL_GTS_SIG1)
            if disable_IL_GTS_SIG1(n)~=1
                text(f_trans((index_center_IL_GTS_SIG1(n)-1),N_fft,fs,nq),...
                    ...
                    {strcat('SIG1ILgts',num2str(n));strcat(num2str(freq_IL_GTS_SIG1(n)/1e6,'%6.2f'),'MHz')},...
                    'FontSize',8);
            end
        end
        if imd_mode==1
            for n=1:length(index_center_IL_GTS_SIG2)
                if disable_IL_GTS_SIG2(n)~=1
                    text(f_trans((index_center_IL_GTS_SIG2(n)-1),N_fft,fs,nq),...
                        ...
                        {strcat('SIG2ILgts',num2str(n));strcat(num2str(freq_IL_GTS_SIG2(n)/1e6,'%6.2f'),'MHz')},...
                        'FontSize',8);
                end
            end
        end
            ...
            for n=1:length(index_center_REF_SPUR_SIG2)
                if disable_REF_SPUR_SIG2(n)~=1
                    if REF_SPUR_SIG2_dbc(n)>-dbc_th_SFDR
                        performance_str8=[performance_str8,'SIG2IL_{gts}',num2str(n),':\color[rgb]{1 0 0}',num2str(REF_SPUR_SIG2_dbc(n),'%6.2f'),'dBc \bf\color[rgb]{0 0 0}'];
                    else
                        performance_str8=[performance_str8,'SIG2IL_{gts}',num2str(n),':',num2str(REF_SPUR_SIG2_dbc(n),'%6.2f'),'dBc \bf'];
                    end
                end
            end
        end
    ...
    if num_interleave~=1
        if imd_mode==1
            performance_str={performance_str1;performance_str2;performance_str3;performance_str4;performance_str5;performance_str6;performance_str7;performance_str8};
        else
            performance_str={performance_str1;performance_str2;performance_str3;performance_str4;performance_str5;performance_str6;performance_str8};
        end
    else
        if imd_mode==1
            performance_str={performance_str1;performance_str2;performance_str3;performance_str6;performance_str7;performance_str8};
        else
            performance_str={performance_str1;performance_str2;performance_str3;performance_str6;performance_str8};
        end
    end
